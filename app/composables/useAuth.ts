...
export const useAuth = () => {
  ...
  const error = ref('')
  const load = async (force = false) => { if (initialized.value && !force) return; try { const r: any = await $fetch('/api/auth/me'); user.value = r.user } catch {} finally { initialized.value = true } }
