...
export const useAuth = () => {
  ...
  const login = async (username: string, password: string) => { loading.value = true; error.value = ''; try { const r: any = await $fetch('/api/auth/login', { method: 'POST', body: { username, password } }); user.value = r.user } catch (e: any) { error.value = e?.data?.statusMessage || '登录失败'; throw e } finally { loading.value = false } }
  const register = async (username: string, password: string, phone: string, code: string) => { loading.value = true; error.value = ''; try { const r: any = await $fetch('/api/auth/register', { method: 'POST', body: { username, password, phone, code } }); user.value = r.user } catch (e: any) { error.value = e?.data?.statusMessage || '注册失败'; throw e } finally { loading.value = false } }
