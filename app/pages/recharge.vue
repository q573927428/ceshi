<template>
  <div class="recharge">
    ...
    <el-dialog v-model="showPay" title="微信扫码支付" width="340px" align-center>
      <div v-if="payment?.codeUrl" class="payment-box"><img class="qr" :src="qrImageUrl" alt="微信支付二维码" /><p>请使用微信扫描二维码完成支付</p><small>订单号：{{ orderNo }}</small><el-button class="copy" @click="copyCodeUrl">复制支付链接</el-button></div>
      <el-alert v-else title="未返回二维码，请检查微信支付配置或稍后重试" type="warning" :closable="false" />
    </el-dialog>
  </div>
</template>
<script setup lang="ts">
const plans = ref<any[]>([]); const message = ref(''); const paying = ref(''); const showPay = ref(false); const orderNo = ref(''); const payment = ref<any>(null)
