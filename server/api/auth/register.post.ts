...
export default defineEventHandler(async (event) => {
  ...
  if (!/^[\w.-]{3,30}$/.test(username) || password.length < 6) throw createError({ statusCode: 400, statusMessage: '用户名至少3位，密码至少6位' })
  if (!/^1\d{10}$/.test(phone) || !verifySmsCode(phone, code)) throw createError({ statusCode: 400, statusMessage: '手机号验证码错误或已过期' })
  try {
    ...
    if (e?.code === 'ER_DUP_ENTRY') throw createError({ statusCode: 409, statusMessage: '用户名或手机号已存在' })
    throw e
  }
})
