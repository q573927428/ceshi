...
import { NEW_USER_COIN_BONUS } from '../../utils/billing'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const result: any = await query('INSERT INTO users (username, phone, password_hash, plan, quota_limit) VALUES (?, ?, ?, "free", ?)', [username, phone, createPasswordHash(password), NEW_USER_COIN_BONUS])
    setSession(event, result.insertId)
    return { user: { id: result.insertId, username, phone, plan: 'free', quotaLimit: NEW_USER_COIN_BONUS, isAdmin: isAdminUser({ username }) } }
  } catch (e: any) {
